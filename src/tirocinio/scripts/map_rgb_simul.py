...
def extract_color_pixels(image_path, color):
    ...
    image[np.where(color_mask > 0)] = [255, 255, 255]

    # Display the original image and the result
    plt.subplot(141), plt.imshow(cv2.cvtColor(image, cv2.COLOR_BGR2RGB)), plt.title('Original Image')
    plt.subplot(142), plt.imshow(color_mask, cmap='gray'), plt.title(f'{color.title()} Pixels Mask')
    plt.subplot(143), plt.imshow(cv2.cvtColor(result, cv2.COLOR_BGR2RGB)), plt.title(f'Extracted {color.title()} Pixels')
    plt.subplot(144), plt.imshow(cv2.cvtColor(image, cv2.COLOR_BGR2RGB)), plt.title(f'Without {color.title()} Pixels')
    plt.show()
