...
for i in range(1,11):
    ...
    df_cov = pd.DataFrame(list(zip(cov_times, cov_areas)), columns =['Time', 'Area'])
    df_exp = pd.DataFrame(list(zip(exp_times, exp_areas)), columns =['Time', 'Area'])
    df_cov.sort_values(by=['Time'])
    df_exp.sort_values(by=['Time'])

    plt.figure(figsize=(8, 6))
    plt.scatter(df_cov['Time'].values, df_cov['Area'].values, marker='o', linestyle='-', label='Coverage')
    plt.scatter(df_exp['Time'].values, df_exp['Area'].values, marker='s', linestyle='--', label='Exploration')
    plt.title(f'Time vs Area {i}')
    plt.xlabel('Time')
    plt.ylabel('Area')
    plt.grid(False)
    plt.legend()
...
df_cov = pd.DataFrame(list(zip(cov_times_tot, cov_areas_tot)), columns =['Time', 'Area'])
df_exp = pd.DataFrame(list(zip(exp_times_tot, exp_areas_tot)), columns =['Time', 'Area'])
print(len(df_cov))
