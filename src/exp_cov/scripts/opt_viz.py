...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Visualize optmization data before or after. ')
    ...
    parser.add_argument('--guards', default="",
        help="Path to the text guards file. If not given, all guard candidates will be shown.", metavar="GUARDS_PATH")
...
def main():
    ...
    if chosen_guards_file_path != "":
        with open(chosen_guards_file_path, "r") as file:
            guard_choice = read_param(file, "guard_choice [*] :=", param_end, "guard_choice", lambda line: line.strip().split()[0], lambda line: True)
        with open(data_file_path, "r") as file:
            guard_position = read_param(file, "param guard_position :=", param_end, "guard_position", lambda line: (line.strip().split()[1], line.strip().split()[2]), lambda line: line.split()[0] in guard_choice)
            file.seek(0,0)
            witness_position = read_param(file, "param witness_position :=", param_end, "witness_position", lambda line: (line.strip().split()[1], line.strip().split()[2]), lambda line: True)
        ...
        with open(data_file_path, "r") as file:
            guard_cost = read_param(file, "param guard_cost :=", param_end, "guard_cost", lambda line: float(line.split()[1]), lambda line: True)
            file.seek(0,0)
            guard_position = read_param(file, "param guard_position :=", param_end, "guard_position", lambda line: (line.strip().split()[1], line.strip().split()[2]), lambda line: True)
            file.seek(0,0)
            witness_position = read_param(file, "param witness_position :=", param_end, "witness_position", lambda line: (line.strip().split()[1], line.strip().split()[2]), lambda line: True)
