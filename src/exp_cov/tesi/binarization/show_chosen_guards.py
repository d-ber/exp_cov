...
def main():
    ...
    guard_scelte_ordine = [0, 3, 5, 1, 2, 4, 6, 8, 7]
    for i in range(len(guardie_scelte)):
        cv2.putText(
        img, #numpy array on which text is written
        str(guard_scelte_ordine.index(i)), #text
        (int(posizioni_guardie[i][0]), int(posizioni_guardie[i][1])), #position at which writing has to start
